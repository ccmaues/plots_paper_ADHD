...
calculate_explained_var <- function(data_frame) {
  if (any(table(data_frame$wave) == 0)) {
    m1 <- DescTools::PseudoR2(glm(diagnosis ~ sex, data = data_frame, family = "binomial"), which = "Nagelkerke") * 100
    m2 <- DescTools::PseudoR2(glm(diagnosis ~ adjusted_PRS, data = data_frame, family = "binomial"), which = "Nagelkerke") * 100
    m3 <- DescTools::PseudoR2(glm(diagnosis ~ popID, data = data_frame, family = "binomial"), which = "Nagelkerke") * 100
    ...
    m16 <- 0  # Set m16 to 0 or some value, or remove it if unnecessary

  } else {
    m1 <- DescTools::PseudoR2(glm(diagnosis ~ sex, data = data_frame, family = "binomial"), which = "Nagelkerke") * 100
    m2 <- DescTools::PseudoR2(glm(diagnosis ~ adjusted_PRS, data = data_frame, family = "binomial"), which = "Nagelkerke") * 100
    m3 <- DescTools::PseudoR2(glm(diagnosis ~ popID, data = data_frame, family = "binomial"), which = "Nagelkerke") * 100
    m4 <- DescTools::PseudoR2(glm(diagnosis ~ wave, data = data_frame, family = "binomial"), which = "Nagelkerke") * 100
    m5 <- DescTools::PseudoR2(glm(diagnosis ~ age, data = data_frame, family = "binomial"), which = "Nagelkerke") * 100
    m6 <- DescTools::PseudoR2(glm(diagnosis ~ p_diagnosis, data = data_frame, family = "binomial"), which = "Nagelkerke") * 100
    m7 <- DescTools::PseudoR2(glm(diagnosis ~ PC1, data = data_frame, family = "binomial"), which = "Nagelkerke") * 100
    m8 <- DescTools::PseudoR2(glm(diagnosis ~ PC2, data = data_frame, family = "binomial"), which = "Nagelkerke") * 100
    m9 <- DescTools::PseudoR2(glm(diagnosis ~ PC3, data = data_frame, family = "binomial"), which = "Nagelkerke") * 100
    m10 <- DescTools::PseudoR2(glm(diagnosis ~ PC4, data = data_frame, family = "binomial"), which = "Nagelkerke") * 100
    m11 <- DescTools::PseudoR2(glm(diagnosis ~ PC5, data = data_frame, family = "binomial"), which = "Nagelkerke") * 100
    m12 <- DescTools::PseudoR2(glm(diagnosis ~ PC6, data = data_frame, family = "binomial"), which = "Nagelkerke") * 100
    m13 <- DescTools::PseudoR2(glm(diagnosis ~ PC7, data = data_frame, family = "binomial"), which = "Nagelkerke") * 100
    m14 <- DescTools::PseudoR2(glm(diagnosis ~ PC8, data = data_frame, family = "binomial"), which = "Nagelkerke") * 100
    m15 <- DescTools::PseudoR2(glm(diagnosis ~ PC9, data = data_frame, family = "binomial"), which = "Nagelkerke") * 100
    m16 <- DescTools::PseudoR2(glm(diagnosis ~ PC10, data = data_frame, family = "binomial"), which = "Nagelkerke") * 100
  }

  for_plot <- data.frame(
    Variables = c(
      "Sex", "PRS", "Site", "Wave", "Age", "Parent\nDiagnosis", 
      "PC1", "PC2", "PC3", "PC4", "PC5", "PC6", "PC7", "PC8", "PC9", "PC10"
    ),
    Explained_variance = c(m1, m2, m3, m4, m5, m6, m7, m8, m9, m10, m11, m12, m13, m14, m15, m16)
  ) %>%
...
colors_and_tags <- c(
  Site = "#1f77b4", Sex = "#ff7f0e", Wave = "#2ca02c",
